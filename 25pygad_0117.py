...
INITIAL_POP_CHANGE_COUNT=10   # 초기 개체군을 만들 때 변경하려는 염기의 개수
INPUT_DIRECTORY = "/home/dna/people/doyoung/PYGAD_DATA/BLASTP_OUTPUT" # BLASTP의 output. target seq
SEQ1_DIRECTORY = "/home/dna/people/doyoung/PYGAD_DATA/SEQ1"  # seq1
...
def load_sequences_from_directory(directory_path, seq1_dir):
    allowed_extensions = (".fa", ".txt")    # txt 파일, fa 파일 모두 input 가능

    seq1_files = [f for f in os.listdir(seq1_dir) if f.endswith(allowed_extensions)]
    if not seq1_files:
        raise ValueError(f"No .fa or .txt files found in SEQ1 directory: {seq1_dir}")

    seq1_dna_sequences = []
    for seq1_file in seq1_files:
        with open(os.path.join(seq1_dir, seq1_file), 'r') as f:
            lines = f.readlines()
            if not lines[0].startswith(">"):                                 
                raise ValueError(f"Invalid FASTA format in {seq1_file}")
            sequence = ''.join(lines[1:]).replace('\n', '').strip()            # fa 파일의 맨 첫 줄 제외(>로 시작하는 부분)

            if is_dna_sequence(sequence):
                seq1_dna_sequences.append(sequence)  
            else:
                seq1_dna_sequences.append(convert_amino_acid_sequence_to_dna(sequence))  

    target_files = [f for f in os.listdir(directory_path) if f.endswith(allowed_extensions)]
    if not target_files:
        raise ValueError(f"No .fa or .txt files found in target directory: {directory_path}")

    target_dna_sequences = []
    for target_file in target_files:
        with open(os.path.join(directory_path, target_file), 'r') as f:
            lines = f.readlines()
            if not lines[0].startswith(">"):
                raise ValueError(f"Invalid FASTA format in {target_file}")
            sequence = ''.join(lines[1:]).replace('\n', '').strip()

            if is_dna_sequence(sequence):
                target_dna_sequences.append(sequence)               # DNA 서열인지 단백질 서열인지 확인 후, 단백질 서열일 경우 변환
            else:
                target_dna_sequences.append(convert_amino_acid_sequence_to_dna(sequence))  

    return seq1_dna_sequences, target_dna_sequences, target_files
...
def run_ga(sequence1, target_sequence, num_generations, sol_per_pop, output_path, target_file, num_parents_mating, gene_space_high):
    custom_mapping = {}
    current_number = [5]

    pad_sequence = target_sequence + ("O" * max(0, (len(sequence1) - len(target_sequence))))
    pad_sequence = pad_sequence[:len(sequence1)]
    pad_sequence2 = [map_char_to_number(char, custom_mapping, current_number) for char in pad_sequence]    

    initial_population = make_initial_population(sequence1, sol_per_pop, custom_mapping, current_number)
    ...
    if best_solution_fitness == len(pad_sequence2):
        ...
        print(f"Target sequence {target_file} could not be reached through evolution.")
        with open(output_path, "a") as f:
            f.write(f"{target_file}\n")
        return False

def compare_sequences(seq1_dir, directory_path, output_directory):
    """
    Compare SEQ1 sequences with the target sequences in the directory.
    """
    seq1_dna_sequences, target_dna_sequences, target_files = load_sequences_from_directory(directory_path, seq1_dir)
    os.makedirs(output_directory, exist_ok=True)
    output_path = os.path.join(output_directory, "pygad_output_seq0117_ver4.txt")

    all_reachable = True
    for seq1_idx, seq1_dna in enumerate(seq1_dna_sequences):
        for idx, (target_dna, target_file) in enumerate(zip(target_dna_sequences, target_files), start=1):
            remaining = len(target_dna_sequences) - idx
            print(f"Comparing SEQ1[{seq1_idx}] with target sequence {target_file} ({remaining} remaining)...")
            gene_space_high = len(target_dna) + 1 if GENE_SPACE_HIGH is None else GENE_SPACE_HIGH
    ...
    if all_reachable:
        print("All sequences could evolve to match the target sequences. No output file generated.")
    else:
...
def main():
    ...
    seq1_dir = SEQ1_DIRECTORY
    output_directory = OUTPUT_DIRECTORY
    try:
        compare_sequences(seq1_dir, directory_path, output_directory)
